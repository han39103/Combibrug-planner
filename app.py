"""
Streamlit front-end for the Workforce Assignment CP Solver
==========================================================
Deploy to Streamlit Cloud:
  1. Push workforce_cp_solver.py, app.py, requirements.txt to GitHub.
  2. Connect repo on share.streamlit.io — set main file to app.py.
"""
...
st.caption("Constraint Programming model — OR-Tools CP-SAT")
...
# ---------------------------------------------------------------------------
# Main panel: run & display
# ---------------------------------------------------------------------------
if run_btn:
    # Use uploaded files or fall back to defaults on disk
    staff_source    = staff_file    if staff_file    else DEFAULT_STAFF
    projects_source = projects_file if projects_file else DEFAULT_PROJECTS
    ...
    c5, c6 = st.columns(2)
    c5.metric("Permanent — contract hrs/month", f"{summary['permanent_contract_hrs']:.1f} h")
    c6.metric("Permanent — assigned hrs",    f"{summary['permanent_assigned_hrs']:.1f} h")

    # Full results table
    st.subheader("Assignment Results")
    st.dataframe(result_df, use_container_width=True, hide_index=True)
    st.subheader("Open Shifts / Uncovered Demand")
    ...
    st.info(
        ...
        "default sample files), set the planning period parameters, then click **Run Solver**."
    )
